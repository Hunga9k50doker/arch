...
class ClientAPI {
  ...
  async log(msg, type = "info") {
    const accountPrefix = `[Arch Network][Account ${this.accountIndex + 1}][${this.itemData.address}]`;
  ...
  async getTasks(category) {
    return this.makeRequest(`${this.baseURL}/missions?category=${category}`, "get");
  }

  async completeTask(id) {
    return this.makeRequest(`${this.baseURL}/missions/${id}/complete`, "post");
  }
  ...
  async handleSyncData() {
    ...
    if (userData.success) {
      const { xp, levelName, level, ore, discordId, discordHandle, xHandle, xId } = userData.data;
      this.log(
        `XP: ${xp.points || 0} | ORE: ${ore.points || 0} | Level: ${level} (${levelName}) | Rank: ${xp?.rank || "Unknow"} | Twitter: ${xId ? xHandle : "Not connnect"} | Discord: ${
          discordId ? discordHandle : "Not connect"
        }`,
        "custom"
      );
    } else {
  ...
  async handleTask() {
    this.log(`Getting tasks...`);
    let tasks = [];
    const categories = ["DAILY", "WEEKLY", "COMMUNITY", "ONBOARDING"];
    for (const cate of categories) {
      const result = await this.getTasks(cate);
      if (!result.success) continue;
      const tasksAvailable = result.data.filter((task) => task.completionStatus === "UNCOMPLETED" && !settings.SKIP_TASKS.includes(task.id));
      tasks = [...tasks, ...tasksAvailable];
    }

    if (tasks.length == 0) return this.log(`No tasks available!`, "warning");
    for (const task of tasks) {
      this.log(`Trying complete task: ${task.id} | ${task.name}...`, "info");
      const resClaim = await this.completeTask(task.id);
      if (resClaim.success && resClaim.data.isCompleted) this.log(`Claim task ${task.id} | ${task.name} success | Reward: ${task.rewardXp} XP & ${task.rewardOre} ORE`, "success");
      else this.log(`Can't claim task ${task.id} | ${task.name} | ${JSON.stringify(resClaim)}`, "warning");
      await sleep(1);
    }
  }
...
async function main() {
  ...
  const data = privateKeys.map((val, index) => {
    ...
    const item = {
      address: payload.walletAddress.ordinals,
      accessToken: val,
